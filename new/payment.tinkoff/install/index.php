...
Class payment_tinkoff extends CModule
{
	...
	function __construct()
	{
		...
		if (is_array($arModuleVersion) && array_key_exists("VERSION", $arModuleVersion)) {
			...
			$tinkoffErrors[] = Loc::getMessage('payment_t__version_info_error');

		$this->MODULE_NAME          = Loc::getMessage("payment_t__MODULE_NAME");
		$this->MODULE_DESCRIPTION   = Loc::getMessage("payment_t__MODULE_DESC");
        $this->PARTNER_NAME         = GetMessage('payment_t__PARTHER_NAME');
        $this->PARTNER_URI          = GetMessage('payment_t__PARTHER_URI');
	}
    ...
    {
        if (!file_exists($from))
            throw new \Bitrix\Main\SystemException(Loc::getMessage('payment-tp__file-not-exists', array(
                '#file#' => $from
            )));
	...
	{
        DeleteDirFilesEx('/bitrix/php_interface/include/sale_payment/payment_tinkoff/');

...
Class rover_tinkoff extends CModule
{
	...
	function DoInstall()
	{
        ...
        if ((count($tinkoffSites) > 1) && ($step < 2)){
            $APPLICATION->IncludeAdminFile(Loc::getMessage("rover-tp__install_title"),
                dirname(__FILE__) . "/step.php");
        } else {

            if (class_exists('\Rover\Tinkoff\Dependence')) {
                $dependence = new Dependence();
                $depErrors  = $dependence->checkBase()->getErrors();

                $tinkoffErrors = array_merge($tinkoffErrors, $depErrors);
            } else
                $tinkoffErrors[] = Loc::getMessage('rover-tp__dependence_error');
            ...
            if (empty($tinkoffErrors)) {
                ...
                if (empty($sites)) {
                    $tinkoffErrors[] = Loc::getMessage('rover-tp__site-id-not-found');
                } else {
            ...
            $APPLICATION->IncludeAdminFile(Loc::getMessage("rover-tp__install_title"), dirname(__FILE__) . '/message.php');
        }
	}
    ...
    function InstallFiles($arParams = array())
    {
        ...
        if (!strlen($docRoot)){
            $tinkoffErrors[] = Loc::getMessage('rover-tp__doc-root-not-found', array(
                '#site-name#' => $siteId
            ));
            return;
        }
        ...
        if (is_dir($source = $docRoot . '/bitrix/modules/' . $this->MODULE_ID . '/install/')) {
            ...
            if (is_dir($dir3) && is_dir($ipn_dir)) {
                ...
                $tinkoffErrors[] = Loc::getMessage('rover-tp__dir-error', array(
                    '#dir#' => $ipn_dir
                ));
            } else {
                $tinkoffErrors[] = Loc::getMessage('rover-tp__dir-error', array(
	...
	function DoUninstall()
	{
        ...
        global $APPLICATION;
		$APPLICATION->IncludeAdminFile(Loc::getMessage("rover-tp__uninstall_title"),

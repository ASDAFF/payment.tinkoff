...
class Dependence
{
	...
	public function checkTrialElapsedDays()
    {
        ...
        if (self::isDemoMode())
            $this->addError(Loc::getMessage('rover-tp__is_trial'));
    ...
    {
        return Loader::includeSharewareModule('rover.tinkoff') == Loader::MODULE_DEMO;
    }
    ...
    {
        return Loader::includeSharewareModule('rover.tinkoff') == Loader::MODULE_DEMO_EXPIRED;
    }
	...
	public function checkTrialExpired()
    {
        if (self::isTrialExpired())
            $this->addError(Loc::getMessage('rover-tp__trial_expired'));
	...
	public function checkPhpVer()
	{
        if (PHP_VERSION_ID < self::MIN_VERSION__PHP)
            $this->addError(Loc::getMessage('rover-tp__php_version_error', array(
	...
	public function checkCurl()
	{
		if (!function_exists('curl_init'))
			$this->addError(Loc::getMessage('rover-tp__no_curl_error'));
	...
	public function checkMainVer()
	{
		if (!CheckVersion(self::getVersion('main'), self::MIN_VERSION__MAIN))
			$this->addError(Loc::getMessage('rover-tp__main-version-error'));
    ...
    public function checkExists($dir)
    {
        if (!file_exists($dir) && !mkdir($dir))
            $this->addError(Loc::getMessage('rover-tp__mkdir-error', array('#dir#' => $dir)));
